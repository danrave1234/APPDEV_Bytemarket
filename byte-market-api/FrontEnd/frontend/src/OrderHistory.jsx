...
const UserOrderHistory = () => {
    ...
    const [orders, setOrders] = useState(() => {
        // Retrieve cached orders from local storage on initial render
        ...
        if (userid) {
            const fetchOrders = async () => {
                try {
                    ...
                    // Only set to localStorage if new orders are fetched
                    if (userOrders.length > 0) {
                        setOrders(userOrders);
                        localStorage.setItem('userOrders', JSON.stringify(userOrders));
                        console.log("Stored orders in localStorage:", localStorage.getItem('userOrders'));
                    }
                } catch (error) {
